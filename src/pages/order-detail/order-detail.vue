...
<view :style="globalThemeStyle" v-if="order" class="page detail-page">
  <view class="detail-nav" :style="`padding-top:${statusHeight}px`">
    <view class="nav-row"><button hover-class="none" class="nav-back page-back detail-back" @tap="back"><image src="/static/assets/icons/back.svg" mode="aspectFit" /></button><text class="nav-title">订单详情</text><view class="nav-back"></view></view>
  </view>
  ...
  <view class="detail-hero" :style="`opacity:${heroOpacity};transform:translateY(-${heroLift}rpx)`">
    ...
    <view class="status-block"><text>{{statusText}}</text><text>{{order.status === 'unpaid' ? '剩余支付时间 ' + paymentCountdown : order.status === 'making' ? '预计 20:10 前送达' : order.status === 'delivery' ? '骑手正在向你飞奔' : order.status === 'cancelled' ? (order.cancelledReason || '订单已取消') : '感谢你的支持'}}</text></view>
    <view class="step-row"><view v-for="(item, index) in steps" :key="item.name" :class="`step ${item.done ? 'done' : ''} ${item.current ? 'current' : ''}`"><view><image v-if="item.done" src="/static/assets/icons/check.svg" mode="aspectFit" /><text v-else>{{index + 1}}</text></view><text>{{item.name}}</text></view></view>
    </view>
  </view>
  <view class="detail-content">
    <view class="card detail-card"><view class="card-title"><text>食刻·品质厨房</text><button hover-class="none" @tap="contact">联系商家</button></view><view v-for="(item, index) in order.items" :key="item.key" class="detail-goods"><view :style="`background:${item.bg}`"><image :src="item.icon" mode="aspectFill" /></view><text>{{item.name}}\n<text>{{item.spec}} ×{{item.count}}</text></text><text>¥{{item.price * item.count}}</text></view><view class="detail-bill"><view><text>商品小计</text><text>¥{{order.goodsTotal}}</text></view><view><text>包装费</text><text>¥{{order.packingFee}}</text></view><view><text>优惠</text><text class="orange">-¥{{order.discount + order.couponDiscount + (order.memberDiscount || 0) + (order.walletDiscount || 0)}}</text></view><view class="paid"><text>实付</text><text>¥{{order.total}}</text></view></view></view>
    <view class="card detail-card"><text class="block-title">配送信息</text><view class="info-row"><text>收货地址</text><text>{{order.address.fullDetail || order.address.detail}}\n{{order.address.name}} {{order.address.phone}}</text></view><view class="info-row"><text>送达时间</text><text>{{order.deliveryTime}}</text></view><view class="info-row"><text>配送服务</text><text>食刻专送</text></view></view>
    <view class="card detail-card"><text class="block-title">订单信息</text><view class="info-row"><text>订单号码</text><view class="order-id-value"><text>{{order.id}}</text><button hover-class="none" @tap="copy">复制</button></view></view><view class="info-row"><text>下单时间</text><text>{{order.createdAt}}</text></view><view class="info-row"><text>支付方式</text><text>{{order.payMethod === 'alipay' ? '支付宝支付' : order.payMethod === 'wallet' ? '食刻钱包支付' : '快捷支付'}}</text></view><view class="info-row"><text>订单备注</text><text>{{order.remark || '无'}}</text></view></view>
    <view v-if="recommendations.length" class="recommend-section">
      ...
      <view class="recommend-grid">
        <view v-for="(item, index) in recommendations" :key="item.id" class="recommend-card" :data-id="item.id" @tap="goFoodDetail">
          <view class="recommend-visual" :style="`background:${item.bg}`">
          ...
          <view class="recommend-info">
            ...
            <view class="recommend-bottom">
              <view><text class="recommend-price">¥{{item.price}}</text><text class="recommend-old">¥{{item.oldPrice}}</text></view>
              <button hover-class="none" :data-id="item.id" @tap.stop="addRecommended">＋</button>
            </view>
          </view>
        </view>
      </view>
  ...
  <view :class="`detail-actions ${order.status === 'unpaid' || order.status === 'making' || order.status === 'delivery' || order.status === 'done' ? 'two-actions' : 'single-action'}`"><button hover-class="none" v-if="order.status === 'unpaid'" class="primary-small" @tap="payNow">立即付款</button><button hover-class="none" v-if="order.status === 'making' || order.status === 'delivery'" class="primary-small" @tap="confirmDelivery">确认送达</button><button hover-class="none" v-if="order.status === 'done'" class="primary-small" @tap="reviewOrder">立即评价</button><button hover-class="none" @tap="contact">联系客服</button></view>
...
// 订单详情页：展示订单进度与明细，维护支付倒计时、推荐商品和订单操作。
...
const pageConfig = {
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      order,
      fromPayResult: options.from === 'payResult',
      recommendations: this.getRecommendations(order)
    })
    this.buildStatus()
  },
  // 每次显示时重新读取订单，确保列表操作和倒计时状态同步。
  onShow() {
  ...
  getRecommendations(order) {
    ...
    const candidates = foodData.foods
      .filter(item => !orderedIds.has(Number(item.id)))
      .map(item => ({ ...item }))
  ...
  buildStatus() {
    ...
    const steps = [
      { name: '订单已提交', done: true }, { name: order.status === 'unpaid' ? '等待付款' : '支付成功', done: order.status !== 'unpaid' },
      { name: '商家制作', done: ['making','delivery','done'].includes(order.status) },
      { name: '骑手配送', done: ['delivery','done'].includes(order.status) }, { name: '已送达', done: order.status === 'done' }
    ].map((item, index) => ({ ...item, current: index === currentStep }))
    this.setData({ statusText: map[order.status], steps })
  },
  startPaymentCountdown() {
    this.stopPaymentCountdown()
    this.updatePaymentCountdown()
    if (this.order && this.order.status === 'unpaid') {
      this.paymentTimer = setInterval(() => this.updatePaymentCountdown(), 1000)
    }
  },
  ...
  updatePaymentCountdown() {
    ...
    this.setData({
      order,
      paymentCountdown: paymentCountdown.formatRemaining(order.paymentDeadline)
    })
  },
  // 滚动时让状态区以更快速度连续淡出并收进标题栏。
  onDetailScroll(e) {
  ...
  back() {
    ...
    uni.navigateBack({
      fail: () => uni.reLaunch({ url: '/pages/orders/orders' })
    })
  },
  copy() { uni.setClipboardData({ data: this.order.id }) },
  payNow() {
    const order = paymentCountdown.getOrder(this.order.id)
  ...
  confirmDelivery() {
    ...
    const orders = store.get('sk_orders', []).map(item => item.id === this.order.id ? { ...item, status: 'done' } : item)
    store.set('sk_orders', orders)
    this.setData({ order: { ...this.order, status: 'done' } }); this.buildStatus()
    uni.showToast({ title: '订单已确认送达', icon: 'success' })
  },
  reviewOrder() { uni.navigateTo({ url: `/pages/review/review?id=${this.order.id}` }) },
  again() { store.set('sk_cart', this.order.items.map(item => ({ ...item, checked: true }))); uni.navigateTo({ url: '/pages/cart/cart' }) }
}
