<template>
	<view class="login-page"
		:style="`${globalThemeStyle};--login-status-height:${statusHeight}px; padding-top:${statusHeight}px`">
		...
		<view class="login-card">
			<view :class="`mode-tabs ${mode === 'login' ? 'login-active' : 'register-active'}`">
				<button hover-class="none" :class="`mode-tab ${mode === 'login' ? 'active' : ''}`" data-mode="login"
					@tap="switchMode">{{ labels.login }}</button>
				<button hover-class="none" :class="`mode-tab ${mode === 'register' ? 'active' : ''}`"
					data-mode="register" @tap="switchMode">{{ labels.register }}</button>
			</view>

			<view :key="formAnimateKey" class="form-panel">
				<view class="input-row">
					<input maxlength="10" :placeholder="labels.usernamePlaceholder" :value="username"
						@input="usernameInput" />
				</view>
				...
				<view class="input-row password-row">
					<input :password="!showPassword" maxlength="20" :placeholder="labels.passwordPlaceholder"
						:value="password" @input="passwordInput" />
					<button hover-class="none" class="eye-btn" data-field="password" @tap="togglePasswordVisible">
						<image :src="showPassword ? '/static/assets/icons/eye-off.svg' : '/static/assets/icons/eye.svg'"
							mode="aspectFit" />
					</button>
				</view>
				<view v-if="mode === 'register'" class="input-row password-row confirm-row">
					<input :password="!showConfirmPassword" maxlength="20" :placeholder="labels.confirmPlaceholder"
						:value="confirmPassword" @input="confirmInput" />
					<button hover-class="none" class="eye-btn" data-field="confirm" @tap="togglePasswordVisible">
						<image
							:src="showConfirmPassword ? '/static/assets/icons/eye-off.svg' : '/static/assets/icons/eye.svg'"
							mode="aspectFit" />
					</button>
				</view>
			</view>
			...
			<view class="agreement">
				...
				<view class="agreement-copy">
					...
					<button hover-class="none" class="agreement-link" data-type="user"
						@tap="openAgreement">{{ labels.service }}</button>
					<text>{{ labels.and }}</text>
					<button hover-class="none" class="agreement-link" data-type="privacy"
						@tap="openAgreement">{{ labels.privacy }}</button>
				</view>
			</view>
		</view>
	</view>
	...
	const pageConfig = {
		...
		switchMode(e) {
			...
			this.setData({
				...
				showPassword: false,
				showConfirmPassword: false,
				formAnimateKey: this.formAnimateKey + 1
			})
		},
		toggleMode() {
		...
		async submit() {
			...
			try {
				...
				await addressBackend.fetchAddresses({
					force: true
				})
				...
				toast((err && (err.errMsg || err.message)) ||
					'\u540e\u7aef\u670d\u52a1\u6682\u65f6\u4e0d\u53ef\u7528')
			} finally {
				this.setData({
					loading: false
...
<style>
	...
	.confirm-row {
		animation-name: confirmExpandIn
	}
	...
	@keyframes inputRiseIn {
		from {
			opacity: 0;
			transform: translateY(22rpx) scale(.985)
		}
	...
	@keyframes confirmExpandIn {
		from {
			opacity: 0;
			transform: translateY(-10rpx) scaleY(.88)
		}
